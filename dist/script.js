...
window.addEventListener("DOMContentLoaded", function () {
  ...
  function del() {
    deleteModalState(); // forms(modalState, del);
  }
        ...
        switch (item.nodeName) {
          ...
          case "INPUT":
            if (item.getAttribute("type") === "checkbox") {
              i === 0 ? state[prop] = "Холодное" : state[prop] = "Tеплое";
...
var checkNumInputs = function checkNumInputs(selector) {
  var inputNumber = document.querySelectorAll(selector);
...
var forms = function forms(state, del) {
  console.log("form");
  ...
  var formPosts = function formPosts(url, data) {
    var res;
    return regeneratorRuntime.async(function formPosts$(_context) {
      while (1) {
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      e.preventDefault();
      var massageForm = document.createElement("div");
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endTime) {
    var t = Date.parse(endTime) - Date.parse(new Date()),
        ...
        days = Math.floor(t / (1000 * 60 * 60 * 24)); // console.log(seconds, minutes, hours, days);
